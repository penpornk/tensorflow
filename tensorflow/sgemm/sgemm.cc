...
#define FLUSH_SIZE 10000000

class CacheFlusher {
 public:
  CacheFlusher() : buffer_(NULL) { buffer_ = new int[FLUSH_SIZE]; }
  ~CacheFlusher() {
    if (buffer_ != NULL) delete[] buffer_;
  }
  void flush() {
    for (int i = 0; i < FLUSH_SIZE; ++i) buffer_[i] = rand();
  }
...
void call_sgemm(float* a, float* b, float* c, const int m, const int n,
  ...
  dnnl_status_t status = dnnl_sgemm(transpose_a, transpose_b, m, n, k, alpha, a,
